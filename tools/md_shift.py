...
custom_headers = {
    ...
    2: lambda x: f"## {x}\n",
    3: lambda x: f"### {x}\n",
    4: lambda x: f"#### {x.strip()}\n",
    5: lambda x: f"##### {x.strip()}\n",
}
    ...
    with open(out_file, "w") as f:
        for line in lines:
            ...
            if line.lstrip().startswith("#"):
                ...
                f.write(custom_headers[min(5, len(header) + shift)](line))
            else:
                f.write(line)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Shift the headers in a markdown file down by a specified amount")
    parser.add_argument("input", help="The input markdown file")
    parser.add_argument("output", help="The output markdown file")
    parser.add_argument("shift", type=int, help="The amount to shift the headers down by")
